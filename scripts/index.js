...
// this function (below) is to accommodate older machines which cannot handle
// css grid
function tableDisplay() {
  ...
  td = addCell(
      tr,
      "<div class='box ai' id='box ai'><label id='testStuff'></label> " +
  ...
  // cell 1
  td = addCell(
      tr, "<div class='box d' id='selectPlayModeSpace'> Play Mode <br>" +
  ...
  // cell 1
  td = addCell(
      tr, "<div class='box e' id='startVerseSpace'>Start<br>" +
              "<select id='startVerseList' name='myDropDn1'> </select> </div>");
  td.setAttribute('id', 'row5_Cell1')
  // cell 2
  td = addCell(
      tr, "<div class='box f' id='endVerseSpace'>End<br>" +
...
// end of the table display, which is to accoomdate older machines which cannot
// handle css grid
...
function gridDisplay() {
  ...
  addElement('div', document.getElementById('wrapper'), [ 'class', 'box ai' ],
             [ 'id', 'box ai' ]);

  // row 1 - box aii
  addElement('div', document.getElementById('wrapper'), [ 'class', 'box aii' ],
             [ 'id', 'box aii' ]);
  addElement('label', document.getElementById('box aii'),
             [ 'id', 'textStuff' ]);
  addElement('label', document.getElementById('box aii'),
             [ 'id', 'textStuff2' ]);
  ...
  addElement('div', document.getElementById('wrapper'), [ 'class', 'box a' ],
             [ 'id', 'box a' ]);
  addElement('label', document.getElementById('box a'),
             [ 'id', 'readerBookLabel' ]);
  document.getElementById('readerBookLabel').innerHTML = "Reader / Book<br>";
  addElement('select', document.getElementById('box a'),
             [ 'id', 'selectReader' ]);

  // row 2 - row b - textSpace (span 2 across; span 5 down)
  addElement('div', document.getElementById('wrapper'), [ 'class', 'box b' ],
             [ 'id', 'textSpace' ]);

  // row 3 - first cell (span=2) - box c - Select Chapter
  addElement('div', document.getElementById('wrapper'), [ 'class', 'box c' ],
             [ 'id', 'selectChapterSpace' ]);
  document.getElementById('selectChapterSpace').innerHTML = "Chapter<br>";
  addElement('select', document.getElementById('selectChapterSpace'),
             [ 'id', 'selectChapter' ]);

  // row 4 - first cell (span 2) - box d - Play Mode
  addElement('div', document.getElementById('wrapper'), [ 'class', 'box d' ],
             [ 'id', 'selectPlayModeSpace' ]);
  document.getElementById('selectPlayModeSpace').innerHTML = "Play Mode<br>";
  addElement(
      'select', document.getElementById('selectPlayModeSpace'),
      [ 'id', 'selectPlayMode' ], [ 'name', 'myDropDn3' ], [
        "title",
        "Select start and end verses BEFORE selecting 'range of verses' mode!"
      ]);

  // row 5 - cell 1 - box e - start verse
  addElement(
      'div', document.getElementById('wrapper'), [ 'class', 'box e' ],
      [ 'id', 'startVerseSpace' ], [
  ...
  addElement('select', document.getElementById('startVerseSpace'),
             [ 'id', 'startVerseList' ], [ 'name', 'myDropDn1' ]);

  // row 5 - cell 2 - box f - end verse
  addElement('div', document.getElementById('wrapper'), [ 'class', 'box f' ],
             [ 'id', 'endVerseSpace' ],
             [ 'title', "Select the last verse of a 'range of verses'." ]);
  document.getElementById('endVerseSpace').innerHTML = "End<br>";
  addElement('select', document.getElementById('endVerseSpace'),
             [ 'id', 'endVerseList' ], [ 'name', 'myDropDn2' ]);

  // row 6 - cell 1 - box g - display text?
  addElement('div', document.getElementById('wrapper'), [ 'class', 'box g' ],
             [ 'id', 'includeTextSpace' ]);
  addElement(
      'label', document.getElementById('includeTextSpace'),
      [ 'id', 'displayTextLabel' ], [
        'title',
        "Uncheck the box if you do not wish the verse text to be displayed."
      ]);
  document.getElementById('displayTextLabel').innerHTML = "Display Text?<br>";
  addElement('input', document.getElementById('includeTextSpace'),
             [ 'id', 'includeText' ], [ 'name', 'text?' ],
             [ "type", "checkbox" ]);
  ...
  // font label
  addElement('div', document.getElementById('wrapper'), [ 'class', 'box h' ],
             [ 'id', 'box h' ]);
  addElement('label', document.getElementById('box h'), [ 'id', 'fontLabel' ]);
  document.getElementById('fontLabel').innerHTML = "Font<br>";
  // font families
  addElement(
      'select', document.getElementById('box h'), [ 'id', 'fontFamilies' ],
      [ 'name', 'myDropdn4' ], [
        "title",
        "Be sure the font you have selected is installed on your machine."
      ]);
  // font size increase
  addElement('button', document.getElementById('box h'), [ 'id', 'fontUp' ]);
  addElement('img', document.getElementById('fontUp'), [ 'id', 'fontUpImage' ],
             [ 'src', './images/fontSzUp.png' ],
             [ "title", "Increase size of font." ]);
  // font size decrease
  addElement('div', document.getElementById('wrapper'));
  addElement('button', document.getElementById('box h'), [ 'id', 'fontDown' ]);
  addElement('img', document.getElementById('fontDown'),
             [ 'id', 'fontDownImage' ], [ 'src', './images/fontSzDn.png' ],
             [ "title", "Reduce size of font." ]);

  // row 7 - cell 1 - box i - colours + logo
  addElement('div', document.getElementById('wrapper'), [ 'class', 'box i' ],
             [ 'id', 'box i' ]);

  // submit button (colours)
  addElement(
      'button', document.getElementById('box i'), [ 'id', 'scrColBtn' ],
      [ 'type', 'submit' ], [
        'title',
        'Colour format: correct name (eg "blue") or HTML format (eg. "#2E57AE") - without the quotes'
      ]);
  document.getElementById('scrColBtn').innerHTML = 'screen colours';

  // logo
  addElement('div', document.getElementById('box i'), [ 'class', 'logoDiv' ],
             [ 'id', 'logoDiv' ]);
  ...
  addElement('div', document.getElementById('wrapper'), [ 'class', 'box j' ],
             [ 'id', 'box j' ]);
  addElement('div', document.getElementById('box j'), [ 'id', 'myControls' ]);

  // repeat btn
  addElement('button', document.getElementById('myControls'),
             [ 'id', 'repeatBtn' ]);
  addElement(
      'img', document.getElementById('repeatBtn'),
      [ 'id', 'repeatPassageImage' ], [ 'src', './images/repeatBtn.png' ], [
        "title",
        "Repeat: 'all' or 'verse' or 'range of verses' => whichever 'mode' you're in."
      ]);

  // previous verse btn
  addElement('button', document.getElementById('myControls'),
             [ 'id', 'previousVerse' ]);
  addElement('img', document.getElementById('previousVerse'),
             [ 'id', 'previousVerseImage' ],
             [ 'src', './images/previousVerse.png' ],
             [ "title", "Go to previous verse." ]);

  // html controls (this is must be defined - but turned off below)
  addElement('audio', document.getElementById('myControls'),
             [ 'id', 'audioID' ], [ 'controls', 'true' ],
             [ "controlslist", "nodownload" ]);
  addElement('source', document.getElementById('audioID'), [ 'id', 'aS' ],
             [ 'src', '' ], [ "type", "audio/mpeg" ]);

  // play/pause btn
  addElement('button', document.getElementById('myControls'),
             [ 'id', 'playPauseBtn' ]);
  addElement('img', document.getElementById('playPauseBtn'),
             [ 'id', 'playImage' ], [ 'src', './images/playBtn.png' ],
             [ "title", "Play or pause audio." ]);

  // next verse btn
  addElement('button', document.getElementById('myControls'),
             [ 'id', 'nextVerse' ]);
  addElement('img', document.getElementById('nextVerse'),
             [ 'id', 'nextVerseImage' ], [ 'src', './images/nextVerse.png' ],
             [ "title", "Go to next verse." ]);

  // last verse btn
  addElement('button', document.getElementById('myControls'),
             [ 'id', 'lastVerse' ]);
  addElement('img', document.getElementById('lastVerse'),
             [ 'id', 'lastVerseImage' ], [ 'src', './images/lastVerseBtn.png' ],
             [ "title", "Go to last verse." ]);

  // current time label
  addElement('label', document.getElementById('myControls'),
             [ 'id', 'curTimeLabel' ]);
  ...
  addElement('input', document.getElementById('myControls'),
             [ 'id', 'volCtrlSlider' ], [ 'type', 'range' ], [ 'min', '0' ],
