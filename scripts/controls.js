...
var fontTable = {
  "font" :
      [ {"family" : "", "txtWindowDefault" : false, "consoleDefault" : false} ]
};
var defaultSettings = {
  "settings" : [ {
    "screenColour" : "",
...
aS.addEventListener('change', function(event) {
  ...
  // 'https://b-s-space.nyc3.digitaloceanspaces.com/media/Rafael/05-Acts/Raf_Acts_12.mp3')
  // {
  // event.respondWith(
  ...
  xhr.onreadystatechange = function() {
    if (xhr.readyState == 4 && xhr.status == 200) {
      ...
      function populateReaders() {
        ...
        } // if no default is set in the file, it will default to the first
          // reader
  ...
  xhr.onreadystatechange = function() {
    if (xhr.readyState == 4 && xhr.status == 200) {
      ...
      function applySettings() {
        ...
        readerBookLabel.innerHTML =
            settingsFile.settings[0].readerBookLabel + "<br>";
        // page colour
...
// ************** load the cues file - loaded every time a new chapter is
// selected ********************
...
// keep track of the current index - tres important! All will revolve around
// this
var cueIndex = 0;
...
// display initial cue if time of first cue < 5 seconds - this will get run at
// the beginning
if (cueFileAvailable == true && includeText.checked &&
    cueFile.chapter[0].cueTime < 5) {
  displayCue(0);
}
...
function displayCue(nextCue) {
  ...
  if (cueFileAvailable && includeText.checked == true) {
    ...
    var textOut =
        x.book + " " + x.chapter + ":" + x.verseNum + "  " + x.cueText;
...
function populatePlayMode() {
  ...
  // Note that the value for "all" = 0; "verse by verse" = 1; and "range of
  // verses" = 2
  for (i in modeList) {
    var opt = document.createElement("OPTION");
    opt.text = modeList[i];
...
function chgFontSize(amt, objId) {
  ...
  // parseFloat gives you just the numerical value, i.e. strips the 'em' bit
  // away
...
helpText.addEventListener('click', function() {
  ...
  newW = window.open('./help.html', 'windowName',
                     'width=' + popW + ',height=' + popH + ',top=' + topPos +
                         ',left=' + leftPos);
})
 ...
 * three (two??) different event listeners are used to handle changes to the
 *selectChapter dropdown this is to accomocate touch screens
 *
 **************************************/

selectChapter.addEventListener('touchstart',
                               function() { updateCurrentChapter(); }, false);
...
function updateCurrentChapter() {
  ...
  // remember there is an extra element in this box - so index starts at 1 in
  // the box but 0 in the file !!!!!! clear out the select boxes
...
audioID.addEventListener('canplay',
                         function() { playImage.src = "./images/playBtn.png"; })
...
scrColBtn.addEventListener('click', function() {
  ...
  function promptForTwo() {
    ...
    newW = window.open('./popup.html', 'windowName',
                       'width=' + popW + ',height=' + popH + ',top=' + topPos +
                           ',left=' + leftPos);
  }
...
selectPlayMode.addEventListener('change', function() {
  ...
  if (selectPlayMode.selectedIndex == 2 & startVerseValue <= endVerseValue) {
    ...
    // reset the counter to the 1st verse in case user has changed end verse but
    // not start verse
    set1stVerse(false);
...
// the user has selected a start verse. This could be a simple resetting of the
// timeer. or the beginning of a range mode
startVerseList.addEventListener('change', function() { set1stVerse(true); },
                                false);
...
nextVerse.addEventListener('click', function() {
  // if play mode is 2 (range of verses) then cueIndex must be less than
  // endverse index
  if (selectPlayMode.selectedIndex == 2 &&
...
previousVerse.addEventListener('click', function() {
  // if play mode is 2 (range of verses) then cueIndex -2 must not be less than
  // startverse index
  if (selectPlayMode.selectedIndex == 2 &&
      cueIndex - 2 < startVerseList.selectedIndex) {
    return;
  }
...
audioID.addEventListener('timeupdate', function() {
  // Note that currentTime is returned by the built in controls in seconds - so
  // I have to change it to milliseconds to compare with cues
  var timeNow = audioID.currentTime * 1000;
  // put out current time to display label
  document.getElementById('curTimeLabel').innerHTML =
      millisToMinutesAndSeconds(timeNow) + totalTrackTime;
  ...
  if (cueFileAvailable == true && includeText.checked &&
      cueIndex < cueFile.chapter.length) {
...
//     if (cueFile.chapter[i].cueTime > timeNow + 50) { // allow 50 msc's, so it
//     doesn't get ahead of me
...
**  Notes:
           ...
           In the select boxes for start verse and end verse, "value" is the
same as chapter[x].cueNdx. It is the index number of the verse. "text" is the
actual name of the verse.
**
**
*************************************************************/
